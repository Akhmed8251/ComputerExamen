...
const StudentScoreListUko = ({ scores, deptName }) => {
  return (
        ...
        </li> */}
        {
            scores.map(student => 
                <li key={student.studentId} className="examen-results__item result-item">
                    ...
                    { student.answerBlank?.totalScore != null && <Link to={'/uko/student-answers'} className="btn" state={{...student, deptName: deptName}}>Посмотреть ответы</Link> }
                </li>
            )
        }
        {/* <li className="examen-results__item result-item">
...
export default StudentScoreListUko